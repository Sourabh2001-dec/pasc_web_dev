...
state_select.addEventListener('change',(e)=>{
  const code = state_select.options[state_select.selectedIndex].getAttribute('value')
  graph_state.stateCode = code
  addData(state_codes[code],graph_state.caseType)
})

case_select.addEventListener('change',()=>{
  const type = case_select.options[case_select.selectedIndex].getAttribute('value')
...
fetch('https://api.covid19india.org/v2/state_district_wise.json')
  ...
  .then(data => {
    data.forEach(state => {
      state_codes[state.statecode] = {name : state.state,
        Confirmed : [],
        Recovered : [],
        Deceased : [],
        Confirmed_date : [],
    ...
    data.states_daily.forEach(info => {
      Object.keys(info).forEach(function(key){
        ...
        if(state){
          ...
          length > 0 ? new_num = state[info.status][ length -1] + parseInt(info[key]) : new_num = parseInt(info[key])
  ...
  function addData(data,type,start = false) {
   ...
   if(!start){
    ...
    document.getElementById("state_name").innerText = state_codes[graph_state.stateCode].name
    document.getElementById("state_conf").innerText = state_codes[graph_state.stateCode].Confirmed[state_codes[graph_state.stateCode].Confirmed.length - 1]
    document.getElementById("state_reco").innerText = state_codes[graph_state.stateCode].Recovered[state_codes[graph_state.stateCode].Recovered.length - 1]
}
